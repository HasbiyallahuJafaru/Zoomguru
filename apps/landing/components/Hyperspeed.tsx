...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }: { effectOptions?: any }) => {
  ...
  useEffect(() => {
    ...
    const distortions: Record<string, any> = {
      ...
      LongRaceDistortion: {
        ...
        getJS: (progress: number, time: number) => {
          ...
          const cp = 0.0125;
          const d = new THREE.Vector3(
            Math.sin(progress * Math.PI * uFreq.x + time) * uAmp.x - Math.sin(cp * Math.PI * uFreq.x + time) * uAmp.x,
            Math.sin(progress * Math.PI * uFreq.y + time) * uAmp.y - Math.sin(cp * Math.PI * uFreq.y + time) * uAmp.y,
            0,
          );
      ...
      deepDistortionStill: {
        ...
        getDistortion: `uniform vec4 uFreq;uniform vec4 uAmp;uniform vec2 uPowY;float nsin(float val){return sin(val)*0.5+0.5;}#define PI 3.14159265358979\nfloat getDistortionX(float progress){return(sin(progress*PI*uFreq.x)*uAmp.x*2.);}float getDistortionY(float progress){return(pow(abs(progress*uPowY.x),uPowY.y)+sin(progress*PI*uFreq.y)*uAmp.y);}vec3 getDistortion(float progress){return vec3(getDistortionX(progress)-getDistortionX(0.02),getDistortionY(progress)-getDistortionY(0.05),0.);}`,
      },
      deepDistortion: {
        uniforms: deepUniforms,
        getDistortion: `uniform vec4 uFreq;uniform vec4 uAmp;uniform vec2 uPowY;float nsin(float val){return sin(val)*0.5+0.5;}#define PI 3.14159265358979\nfloat getDistortionX(float progress){return(sin(progress*PI*uFreq.x+uTime)*uAmp.x);}float getDistortionY(float progress){return(pow(abs(progress*uPowY.x),uPowY.y)+sin(progress*PI*uFreq.y+uTime)*uAmp.y);}vec3 getDistortion(float progress){return vec3(getDistortionX(progress)-getDistortionX(0.02),getDistortionY(progress)-getDistortionY(0.02),0.);}`,
        getJS: (progress: number, time: number) => {
    ...
    class CarLights {
      ...
      init() {
        ...
        let colors = Array.isArray(this.colors) ? this.colors.map((c: any) => new THREE.Color(c)) : new THREE.Color(this.colors);
    ...
    class LightsSticks {
      ...
      init() {
        ...
        let colors = Array.isArray(options.colors.sticks) ? options.colors.sticks.map((c: any) => new THREE.Color(c)) : new THREE.Color(options.colors.sticks);
    ...
    class App {
      options: any; container: any; hasValidSize: boolean; renderer: any; composer: any; camera: any; scene: any; fogUniforms: any; clock: any; assets: any; disposed: boolean;
      ...
      onWindowResize: any; tick: any; init: any; setSize: any; onMouseDown: any; onMouseUp: any; onTouchStart: any; onTouchEnd: any; onContextMenu: any; renderPass: any; bloomPass: any;

      constructor(container: HTMLElement, options: any) {
        this.options = options.distortion == null ? { ...options, distortion: { uniforms: distortion_uniforms, getDistortion: distortion_vertex } } : options;
      ...
      dispose() {
        this.disposed = true;
        if (this.scene) { this.scene.traverse((obj: any) => { if (!obj.isMesh) return; if (obj.geometry) obj.geometry.dispose(); if (obj.material) { Array.isArray(obj.material) ? obj.material.forEach((m: any) => m.dispose()) : obj.material.dispose(); } }); this.scene.clear(); }
  ...
  return <div id="hyperspeed-canvas" ref={hyperspeed} />;
};

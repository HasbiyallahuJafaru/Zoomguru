      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '32px', maxWidth: '780px' }}>
        ...
        <div className="glass" style={{ borderRadius: '16px', padding: '28px 32px' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px' }}>
            <div>
              <p style={{ fontSize: '12px', color: 'rgba(0,0,0,0.4)', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '6px' }}>Current Plan</p>
              <h2 style={{ fontSize: '28px', fontWeight: 700, color: '#111', letterSpacing: '-0.5px' }}>
            ...
            {sub?.isPro && (
              <div style={{ textAlign: 'right' }}>
                {sub.plan === 'lifetime' ? (
                  ...
                  <>
                    <p style={{ fontSize: '12px', color: 'rgba(0,0,0,0.35)', marginBottom: '4px' }}>Next billing</p>
                    ...
                    <p style={{ fontSize: '13px', color: 'rgba(0,0,0,0.4)', marginTop: '2px' }}>₦15,000 / month</p>
                  </>
                ) : null}
              </div>
            )}
          ...
          <div style={{ marginTop: '24px', paddingTop: '20px', borderTop: '1px solid rgba(0,0,0,0.07)', display: 'flex', gap: '32px', flexWrap: 'wrap' }}>
            {!sub?.isPro ? (
              <>
              ...
              <div>
                <p style={{ fontSize: '12px', color: 'rgba(0,0,0,0.35)', marginBottom: '4px' }}>Device fingerprint</p>
                <p style={{ fontFamily: 'monospace', fontSize: '14px', color: 'rgba(0,0,0,0.55)', background: 'rgba(0,0,0,0.005)', padding: '4px 10px', borderRadius: '6px', display: 'inline-block' }}>
        ...
        {!isLifetime && (
          <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
              ...
              {/* Currency toggle */}
              <div style={{ display: 'flex', background: 'rgba(0,0,0,0.006)', borderRadius: '8px', padding: '3px' }}>
                {(['NGN', 'USD'] as const).map(c => (
                  <button key={c} onClick={() => setCurrency(c)} style={{
                    padding: '5px 14px', borderRadius: '6px', border: 'none',
                    background: currency === c ? 'rgba(0,0,0,0.12)' : 'transparent',
                    color: currency === c ? '#fff' : 'rgba(0,0,0,0.4)',
          ...
          <div className="glass" style={{ borderRadius: '12px', overflow: 'hidden', minWidth: '420px' }}>
            ...
            {FEATURES.map((f, i) => (
              <div key={f.label} style={{
                ...
                <span style={{ fontSize: '14px', color: 'rgba(0,0,0,0.65)' }}>{f.label}</span>
...
function PageTitle() {
  return (
    <div>
      <h1 style={{ fontSize: '24px', fontWeight: 700, color: '#111', letterSpacing: '-0.3px' }}>Subscription</h1>
      <p style={{ color: 'rgba(0,0,0,0.35)', fontSize: '14px', marginTop: '4px' }}>Manage your plan and billing</p>
    </div>
  );
}
...
function UsageStat({ label, value, max }: { label: string; value: number; max: number }) {
  ...
  return (
    <div style={{ minWidth: '140px' }}>
      <p style={{ fontSize: '12px', color: 'rgba(0,0,0,0.35)', marginBottom: '4px' }}>{label}</p>
      <p style={{ fontSize: '18px', fontWeight: 600, color: '#1a1a1a' }}>{value} <span style={{ fontSize: '13px', color: 'rgba(0,0,0,0.35)', fontWeight: 400 }}>of {max}</span></p>
      <div style={{ marginTop: '6px', height: '3px', borderRadius: '2px', background: 'rgba(0,0,0,0.1)', overflow: 'hidden' }}>
...
function PlanCard({ name, price, period, cta, badge, primary, loading, onPay }: {
  ...
  return (
    <div className={primary ? '' : 'glass'} style={{
      ...
      border: primary ? '1px solid rgba(79,110,247,0.4)' : '1px solid rgba(0,0,0,0.08)',
      ...
      {badge && (
        <span style={{
          ...
          background: '#4f6ef7', color: '#111', fontSize: '11px', fontWeight: 600,
          padding: '3px 10px', borderRadius: '20px',
        }}>{badge}</span>
      )}
      <div>
        <p style={{ fontSize: '13px', color: 'rgba(0,0,0,0.5)', marginBottom: '6px' }}>{name}</p>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: '2px' }}>
          <span style={{ fontSize: '28px', fontWeight: 700, color: '#111' }}>{price}</span>
          <span style={{ fontSize: '13px', color: 'rgba(0,0,0,0.4)' }}>{period}</span>
        </div>
      </div>
      <button
        onClick={onPay}
        ...
        style={{
          ...
          background: primary ? '#4f6ef7' : 'rgba(0,0,0,0.08)',

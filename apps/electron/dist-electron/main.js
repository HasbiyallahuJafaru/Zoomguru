...
function initSpeech(win) {
  electron.ipcMain.handle("speech:start", async () => {
    return "";
  });
  electron.ipcMain.handle("speech:stop", async () => {
  });
}
...
if (process.defaultApp) {
  ...
  electron.app.setAsDefaultProtocolClient("zoomguru");
}
let mainWindow = null;
let tray = null;
...
function applyScreenShareExclusion(win) {
...
function createWindow() {
  ...
  if (savedPos && savedPos.x >= 0 && savedPos.x < width - 100 && savedPos.y >= 0 && savedPos.y < height - 100) {
...
async function runProtectionSelfTest() {
  if (!mainWindow) return;
  ...
  try {
    ...
    if (!isProtected) {
      console.warn(
        `⚠️ ZoomGuru overlay may be visible to screen capture. Pixel at (${clampedX},${clampedY}): rgb(${r},${g},${b})`
      );
    } else {
      console.log(
        `✅ Screen protection active [${process.platform}]. Capture pixel: rgb(${r},${g},${b})`
      );
    }
  } catch (err) {
...
function registerIpcHandlers() {
  ...
  electron.ipcMain.handle("device:fingerprint", () => {
    return fingerprint;
  });

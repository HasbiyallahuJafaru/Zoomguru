...
hooks:
  pre: |
    echo "ðŸ” PII Detector scanning for sensitive data..."
  post: |
...
async function scanForPII(content: string, source: string) {
  ...
  if (result.piiFound) {
    console.log(`âš ï¸ PII detected in ${source}`);
...
if (result.hasPII) {
  console.log('ðŸš¨ Action required: Remove or encrypt sensitive data');
}
```

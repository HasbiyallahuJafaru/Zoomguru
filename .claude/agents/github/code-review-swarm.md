...
hooks:
  pre: |
    echo "ðŸš€ [Code Review Swarm] starting: $TASK"
    ...
    if [ -n "$SIMILAR_REVIEWS" ]; then
      echo "ðŸ“š Found ${SIMILAR_REVIEWS} similar successful review patterns"
  ...
  post: |
    echo "âœ¨ [Code Review Swarm] completed: $TASK"
    ...
    if [ "$SUCCESS" = "true" ] && [ "$REWARD" -gt "0.9" ]; then
      echo "ðŸ§  Training neural pattern from successful code review"
...
if (similarReviews.length > 0) {
  console.log('ðŸ“š Learning from past successful reviews:');
...
if (failedReviews.length > 0) {
  console.log('âš ï¸  Avoiding past review mistakes:');
...
## ðŸŽ¯ GitHub-Specific Review Optimizations
...
gh pr comment 123 --body "ðŸ” Multi-agent code review initiated"
```

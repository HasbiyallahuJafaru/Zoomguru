...
hooks:
  pre: |
    echo "ðŸš€ [Issue Tracker] starting: $TASK"
    ...
    if [ -n "$SIMILAR_ISSUES" ]; then
      echo "ðŸ“š Found ${SIMILAR_ISSUES} similar successful issue patterns"
  ...
  post: |
    echo "âœ¨ [Issue Tracker] completed: $TASK"
    ...
    if [ "$SUCCESS" = "true" ] && [ "$REWARD" -gt "0.9" ]; then
      echo "ðŸ§  Training neural pattern from successful issue management"
...
if (similarIssues.length > 0) {
  console.log('ðŸ“š Learning from past successful triages:');
...
if (triageFailures.length > 0) {
  console.log('âš ï¸  Avoiding past triage mistakes:');
...
mcp__github__add_issue_comment {
  ...
  ### Current Status
  - ðŸ”„ Documentation review in progress
  - ðŸ“Š Integration score: 89% (Excellent)

...
hooks:
  pre: |
    ...
    if [ -n "$SIMILAR_PATTERNS" ]; then
      echo "ðŸ“š Found similar successful code patterns (HNSW-indexed)"
    ...
    if [ -n "$FAILURES" ]; then
      echo "âš ï¸  Avoiding past mistakes from failed implementations"
    fi
  ...
  post: |
    ...
    if [ "$SUCCESS" = "true" ] && [ "$TESTS_PASSED" -gt 90 ]; then
      echo "ðŸ§  Training neural pattern from successful implementation"
...
if (similarCode.length > 0) {
  console.log('ðŸ“š Learning from past implementations (HNSW-indexed):');
...
if (failures.length > 0) {
  console.log('âš ï¸  Avoiding past mistakes (EWC++ protected):');
...
## âš¡ Performance Optimization with Flash Attention

...
hooks:
  pre: |
    ...
    if [ -n "$SIMILAR_PLANS" ]; then
      echo "ðŸ“š Found similar successful planning patterns (HNSW-indexed)"
    ...
    if [ -n "$FAILED_PLANS" ]; then
      echo "âš ï¸  Learning from past planning failures"
    fi
  ...
  post: |
    ...
    if [ "$SUCCESS" = "true" ] && [ "$TASKS_COUNT" -gt 10 ]; then
      echo "ðŸ§  Training neural pattern from comprehensive plan"
...
if (similarPlans.length > 0) {
  console.log('ðŸ“š Learning from past planning patterns (HNSW-indexed):');
...
## ðŸ¤ Multi-Agent Planning Coordination
